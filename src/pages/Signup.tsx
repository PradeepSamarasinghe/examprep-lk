...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error } = await signUp(email, password, { display_name: fullName, school });
    setLoading(false);
    if (error) {
        ...
        <div className="glass-strong rounded-2xl p-8 gradient-border">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-lg bg-primary text-primary-foreground font-semibold text-sm text-center hover:bg-primary/90 transition-colors glow-blue mt-2 disabled:opacity-50 flex items-center justify-center gap-2"
            >
